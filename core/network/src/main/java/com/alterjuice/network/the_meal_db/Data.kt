package com.alterjuice.network.the_meal_db
...
fun getTheMealDBItems(): List<TheMealDBItemDTO> {
    val g = Gson()
    val x = TypeToken.getParameterized(List::class.java, TheMealDBItemDTO::class.java)
    val items = g.fromJson<List<TheMealDBItemDTO>>(jsonData, x.type)
    return items
}

fun getTheMealDBItemsAsYumHubMeals(): List<YumHubMeal> {
    return getTheMealDBItems().map { it.toYumHubMeal() }
}

internal val jsonData = """[
     {
        "id": "2",
